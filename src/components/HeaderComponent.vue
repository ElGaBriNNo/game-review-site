<template>
  <div id="header">
    ...
    <div id="textForm">
      <form>
        <input class="form-control" id="search" type="text" name="text" v-model="searchQuery" placeholder="Search for a game" v-on:paste="getSearchResults"/>
      </form>
    </div>
    <div id="icons">
...
import { fetchByName, fetchGenres, GamesF } from "@/services/GameDBapi.js";
...
SearchComponent,
  ...
  methods: {
   ...
   /* searchByName(){
      if(this.searchQuery){
        return CarouselComponent.games.filter((game)=>{
          return this.searchQuery.toLowerCase().split(' ').every(v => game.name.toLowerCase().includes(v))
...
*/
   ...
   //  await this.$store.state.searchList.clean;
      await this.searchQuery.value;
    this.games = await fetchByName(this.searchQuery);
    this.$store.commit("addToSearchList", this.games);
